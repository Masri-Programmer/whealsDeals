...
const Rent = () => {
    ...
    useEffect(() => {
        doGetCars({ page: 1, params, ids: subcategoriesIds, isBuy: 0, min, max })
        dispatch(resetSubCategoriesIds())
    }, []);

    // ...GET CARS
    useEffect(() => {
        doGetCars({ page, params, ids: subcategoriesIds, isBuy: 0, min, max })
        if (changed) {
            doGetCars({ page, params, ids: subcategoriesIds, isBuy: 0, min, max })
            dispatch(resetPage())
            setChanged(false)
    ...
    // ...INFINITE SCROLL
    const fetchMoreData = () => {
        fetchCars({ page: page + 1, params, ids: subcategoriesIds, isBuy: 0, min, max })
            .then((newData) => {
    ...
    const handleSubmit = (d) => {
        ...
        setSearch(d)
        doGetCars({ page: 1, params: d, ids: subcategoriesIds, isBuy: 0, min, max })
        dispatch(resetPage())
    }
    ...
    return (
        <SectionLayout section={"Rent your own Wheels"} >
            <SearchLayout
